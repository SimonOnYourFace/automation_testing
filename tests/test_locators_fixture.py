...
def test_locator_role(page: Page):
    # page.goto("https://www.litres.ru/")
    page.get_by_role("link", name="Вебтуны - эксклюзивно на Литрес!").click()
    # expect(page).to_have_title("Рекомендации для вас- ищите на Литрес")
    expect(page).to_have_title("Лучшие книги в жанре Вебтун – скачать или читать онлайн бесплатно на Литрес")
    page.get_by_role("button", name="Найти").click()
    expect(page.get_by_text("стивен кинг")).to_be_visible()

def test_locator_placeholder(page: Page, open_litres):
    # page.goto("https://www.litres.ru/")
    page.get_by_placeholder("Искать на Литрес").fill("Гарри Поттер")
    page.keyboard.press("Enter")
    expect(page.get_by_text("Результаты поиска «гарри поттер»")).to_be_visible()
